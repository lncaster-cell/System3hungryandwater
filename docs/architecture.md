...
- `BuildMerchantTradeGui(...)` готовит строки UI (заголовок + линии) для клиентской GUI-отрисовки торговли.
- Единые ключи цен/остатков/инвентаря/GUI через helper-функции.
...
### 5) Межрепозиторная совместимость (`nwscript/ambientlive2_bridge.nss`)
...
- `AL2ExportMerchantItem/AL2ImportMerchantItem` синхронизируют торговые прайс/остаток по item-id.
