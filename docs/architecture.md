...
### 5) Сцепка world map ↔ лагерь (`src/world_system.py`)
- `TravelSystem.project_travel(...)` превращает длительность маршрута в «игровые дни» и время ожидания в лагере (по умолчанию 1 день = 1 минута).
- Каждому дню соответствует фиксированное число проверок встреч (`encounter_checks_per_day`), например 5 дней = 10 проверок.
- Если личного лагеря нет, применяется штраф к длительности (`no_camp_penalty_milli`) и сцена отдыха переключается на `inn` (таверна/постоялый двор).
- `CampSystem` хранит факт получения личного лагеря, склад лагеря и закреплённых спутников.
- `EncounterSystem.events_for_checks(...)` генерирует детерминированные события по числу проверок для route.

## Сложность
- Лагерь: `O(P)` по членам партии только в момент переключения.
- Старт/финиш путешествия: `O(1)`.
- Проверка встречи: `O(1)` на одну проверку или `O(K)` на K фиксированных проверок маршрута.
- Покупка строки: `O(1)`.
